...
class App:
    ...
    def run(self):
        ...
        for tamano in self.tamanos:
            arr = self.arreglo_base[:tamano].copy()  

            for nombre, algoritmo in self.algoritmos.items():
                tiempo = self.benchmarking.medir_tiempo(algoritmo, arr.copy())  
                self.resultados.append((tamano, nombre, tiempo))
                print(f"Tamano: {tamano}, Algoritmo: {nombre}, Tiempo: {tiempo:.4f} segundos")  
